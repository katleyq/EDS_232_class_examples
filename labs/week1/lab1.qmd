...
plot1 <- ggplot(data = pumpkins, aes(y = price, x = day)) +
  geom_point()
plot1
```
...
Answer: This plot could be a lot more useful. I notice that there looks to be clusters of similarly priced pumpkins. This indicates that we should explore groupings.

How do we make it more useful? To get charts to display useful data, you usually need to group the data somehow.

```{r}
new_pumpkins %>% 
  ggplot(mapping = aes(x = day, y = price, color = variety)) +
  geom_point(size = 1.6)
```
...
# Pre-processing data for modelling using recipes
